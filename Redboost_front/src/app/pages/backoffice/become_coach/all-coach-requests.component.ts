            ...
            <table *ngIf="!loading && !error && requests.length > 0" class="requests-table">
                ...
                <tbody>
                    <tr *ngFor="let request of requests">
                        ...
                        <td>
                            <button *ngIf="request.status === 'PENDING'" class="approve-btn" (click)="approveRequest(request.id)">Approve</button>
                            <button *ngIf="request.status === 'PENDING'" class="reject-btn" (click)="rejectRequest(request.id)">Reject</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            <div *ngIf="!loading && !error && requests.length === 0" class="no-data">No coach requests found.</div>
        </div>
    `,
    styles: [
        `
